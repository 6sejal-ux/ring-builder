            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 bg-card rounded">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-heading text-base font-semibold truncate">{item.product.node.title}</h4>
                        <p className="text-xs text-muted-foreground font-body">{item.selectedOptions.map(o => o.value).join(' • ')}</p>
                        <p className="font-body font-semibold mt-1">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
